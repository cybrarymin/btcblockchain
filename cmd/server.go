...
func init() {
	...
	serverCmd.PersistentFlags().StringVar(&CmdJaegerPortFlag, "jeager-port", "4317", "Jaeger server port for sending opentelemetry traces")
	...
	serverCmd.PersistentFlags().DurationVar(&CmdSpanExportInterval, "jeager-trace-exporter-intervals", time.Second*1, "intervals which tracer batch exporter will send the traces to the jeager")
	...
	serverCmd.PersistentFlags().StringVar(&CmdGrpcPort, "grpc-listen-port", "4317", "grpc server port to listen on")
